...
export async function getPostImages(page, post) {
  let isImgAvailable = {
    bg: false,
    preview: false,
  };

  isImgAvailable.bg = await fs.promises
    .access(`./public/${page}/preview-bg.png`, fs.constants.F_OK)
    .then(() => true)
    .catch(() => false);

  isImgAvailable.preview = await fs.promises
    .access(`./public/${page}/${post}/preview.png`, fs.constants.F_OK)
    .then(() => true)
    .catch(() => false);
